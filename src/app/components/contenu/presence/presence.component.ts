...
export class PresenceComponent implements OnInit {
  ...
  handleEventClick(arg: any) {
    if (this.isAdmin && arg.event._def.interactive) {
      ...
      this.presence.numberOfHours = arg.event.extendedProps[0].numberOfHours;
    ...
    if (this.presence.id) {
      this.presenceService
        ...
        .subscribe(
          (r) => {
            this.presence = r;
            this.events[this.findIndexById(r.id)] = this.mapPresenceToEvent(r);
            this.events = [...this.events];
            this.dialog = false;
          },
          (e) => {
  ...
  getUserCalendar(id: number) {
    ...
    this.congeService.getAllCurrentConges(this.selectedEmployee.id!).subscribe({
      next: (r) => {
        let conges: Conge[] = r;
